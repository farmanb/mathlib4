...
* `coker_of r` : The functor that assigns to `X : C` the cokernel object associated to `r X ⟶ X`
    (developed in `CokernelConstruction.lean`).
* `r.colon s` : Stenström's `r : s`, constructed as a pullback (developed in `Colon.Lean`).
* `r.π X`: The projection `X ⟶ cokernel (r X)`.
...
def is_idempotent (r : Preradical C) : Prop := r.F ⋙ r.F = r.F

/-- The natural transformation `η : r ⟶ 𝟭 (C)` is always `Mono` because
`mono_app : ∀ X : C, Mono (η.app X)` -/
...
r.map (f ≫ g) = r.map f ≫ r.map g := r.F.map_comp f g

@[simp]
lemma ι_eq_app (r : Preradical C) (X : C) : r.η.app X = r.ι X := rfl

@[simp, reassoc]
lemma ι_naturality (r : Preradical C) {X Y : C} (f : X ⟶ Y) :
r.map f ≫ r.ι Y = r.ι X ≫ f:= r.η.naturality f
