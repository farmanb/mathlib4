...
noncomputable
def toColon_app (r s : Preradical C) (X : C) : r X ⟶ (r.colon s) X :=
...
lemma toColon_app_comp_colon_fst (r s : Preradical C) (X : C) :
    r.toColon_app s X ≫ (r.colon s).ι X = r.ι X := by
...
lemma toColon_app_naturality (r s : Preradical C) {X Y : C} (f : X ⟶ Y) :
  ...
  · calc
    ...
    _ = r.map f ≫ r.ι Y := by rw [toColon_app_comp_colon_fst r s Y]
    _ = r.ι X ≫ f := ι_naturality r f
    _ = (r.toColon_app s X ≫ (r.colon s).ι X )≫ f := by rw [toColon_app_comp_colon_fst]
    ...
    _ = (r.toColon_app s X ≫ (r.colon s).map f) ≫ pullback.fst (r.π Y) (s.ι (r.coker Y)) := rfl
  · calc
    _ = (r.map f ≫ r.toColon_app s Y) ≫ r.colon_snd s Y := rfl
    _ = r.map f ≫ r.toColon_app s Y ≫ r.colon_snd s Y := by simp
    _ = 0 := by simp
    _ = r.toColon_app s X ≫ r.colon_snd s X ≫ s.map (r.coker_map f) := by simp [← Category.assoc]
    _ = r.toColon_app s X ≫ (r.colon s).map f ≫ r.colon_snd s Y := by rw [colon_snd_naturality]
    _ = (r.toColon_app s X ≫ (r.colon s).map f) ≫ r.colon_snd s Y := by rw [Category.assoc]
    _ = (r.toColon_app s X ≫ (r.colon s).map f) ≫ pullback.snd (r.π Y) (s.ι (r.coker Y)) := rfl
