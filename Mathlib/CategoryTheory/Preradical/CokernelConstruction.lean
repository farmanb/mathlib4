...
noncomputable abbrev quotient (r : Preradical C) : C ⥤ C := cokernel r.η

/-- The canonical projection onto the cokernel of `r.ι X : r X ⟶ X`. -/
noncomputable def π (r : Preradical C) (X : C) : X ⟶ r.quotient.obj X := (cokernel.π r.η).app X
...
@[simp, reassoc]
lemma π_app_comp_map_η (r : Preradical C) (X : C) :
    (cokernel.π r.η).app (r X) ≫ r.quotient.map (r.η.app X) = 0 := by
...
`r.ι X`. -/
noncomputable
def quotientObjIso (r : Preradical C) (X : C) : r.quotient.obj X ≅ cokernel (r.ι X) := by
  simpa using (CategoryTheory.Limits.PreservesCokernel.iso
    ((CategoryTheory.evaluation C C).obj X) r.η)

lemma π_comp_quotientObjIso (r : Preradical C) (X : C) :
    r.π X ≫ (r.quotientObjIso X).hom = cokernel.π (r.ι X) := by
...
noncomputable
def isKernel_ι (r : Preradical C) (X : C) :
...
noncomputable
def isoKernel_π (r : Preradical C) (X : C) : r X ≅ kernel (r.π X) := by
  simpa using
