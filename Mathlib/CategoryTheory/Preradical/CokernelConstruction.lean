...
/-- The cokernel of the structure morphism `r.ι X : r X ⟶ X`. -/
noncomputable
def coker (r : Preradical C) (X : C) := (cokernel_of r).obj X

/-- The map on cokernels induced by a morphism `f : X ⟶ Y`.
This is the action of the functor `cokernel_of r` on morphisms. -/
noncomputable
...
/-- The canonical cokernel projection `X ⟶ r.coker X` associated to `r.ι X`. -/
noncomputable
...
/-- The natural transformation `𝟭 C ⟶ cokernel_of r` whose components are the cokernel projections
`r.π X : X ⟶ r.coker X`. -/
noncomputable
def coker_η (r : Preradical C) : 𝟭 C ⟶ cokernel_of r where
  ...
  naturality := by
    intro X Y f
    have h : r.ι X ≫ f ≫ cokernel.π (r.ι Y) = 0 := by
      rw [← Category.assoc, r.ι_naturality f, Category.assoc, cokernel.condition, comp_zero]
    symm
    exact cokernel.π_desc (r.ι X) (f ≫ cokernel.π (r.ι Y)) h
...
@[simp]
lemma ι_comp_π (r : Preradical C) (X : C) : r.η.app X ≫ r.π X = 0 := by
...
lemma fromKernel_π_comp (r : Preradical C) (X : C) :
    r.fromKernel_π X ≫ (r.η.app X) = kernel.ι (r.π X) :=
...
r.toKernel_π X ≫ r.fromKernel_π X = 𝟙 (r X) := by
  ...
  simp only [coker_eq, ι_eq_app, Category.assoc, fromKernel_π_comp, toKernel_π_comp_kernel_ι]
...
r.fromKernel_π X ≫ r.toKernel_π X = 𝟙 (kernel (r.π X)) := by
  ...
  simp only [coker_eq, Category.assoc, toKernel_π_comp_kernel_ι, ι_eq_app, fromKernel_π_comp]
